...
def split_data(data_path, type, filter_num):      
    path = './testdata/NCyc_esm2-1.pkl'
    data = pk.load(open(path,"rb"))
    keys=[]
    for key, _ in data.items(): 
            keys.append(key.split('_')[0])
    keys_stack = np.stack(keys) #(219090,)
    
    data = pk.load(open(data_path,"rb")) #DeviceArray type
    data = np.array(data) #(219090,4107)
    X = data #(1,1965, 4107)
    y = keys_stack
    
    csv_path = '../../../src/datasets/Large-Scale/NCycDB/NCyc_num.csv'
    df = pd.read_csv(csv_path)
    df = df.drop_duplicates(subset='SeqID', keep='first')
    df['SeqID'] = df['SeqID'].astype(str).apply(lambda x: x.split('_')[0])
    y_output = df.set_index('SeqID').loc[y]['Cate Code'].values   
    ...
    train_split_path = "./NCyc_"+type+".pkl"
...
def train(data_path, type, filter_num, report_logger):
    ...
    report_logger.info("Dataset: NCyc")
    ...
    result_path = os.path.join(report_dir, f'NCyc_{type}_classification_results.csv')
    print(f"Result path: {result_path}")
    print('-'*70)
    
    results_df.to_csv(result_path, index=False)
    results_df.to_csv(result_path, index=False)
    original_df = pd.read_csv( '../../../src/datasets/Large-Scale/NCycDB/NCyc_code.csv')
    encoded_df = pd.read_csv( '../../../src/datasets/Large-Scale/NCycDB/NCyc_num.csv')
    ...
    result_path_updated = f'{figure_path}/classification_report/NCyc_{type}_classification_results_updated.csv'
...
data_path = './testdata/NT_NCyc2.pkl'
split_data(data_path, type, filter_num)
# train_ML(data_path)

data_path = f'./NCyc_{type}.pkl'
