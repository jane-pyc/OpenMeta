...
def split_data(data_path, type, filter_num):       
    path = './testdata/ENZYME_esm2-1.pkl'
    data = pk.load(open(path,"rb"))
    keys=[]
    for key, _ in data.items(): # values.hape=(1965,626,4107)
            keys.append((key.split('_')[1]))
    keys_stack = np.stack(keys) #(1965,)
    data = pk.load(open(data_path,"rb"))
    values=[]
    for _, value in data.items(): # values.hape=(1965,626,4107)
            values.append(value.mean(1)) #(1965,4107)
    keys_stack = np.stack(keys)
    values_stack = np.stack(values)
    
    X = values_stack #(1,1965, 4107)
    X = X[0]
    y = keys_stack    
    ...
    y_output = df.set_index('SeqID').loc[y]['Cate Code'].values #(4230,)    
   
    y = y_output    
    ec_counter = Counter(y)
    y_filter = [ec for ec, c in ec_counter.items() if c < filter_num ]
    y_new = np.delete(y,np.where(np.isin(y,y_filter))[0],axis =0) #(1734,)
    ...
    train_split_path = "./ENZYME_"+type+".pkl"
...
def train(data_path, type, filter_num, report_logger):
    ...
    report_logger.info("Dataset: ENZYME")
    ...
    for class_name, metrics in report_dict.items():
        if class_name not in ['accuracy', 'macro avg', 'weighted avg']:
            ...
            results_df = results_df.concat(results_df, pd.DataFrame({'Class Name': class_name, 
...
data_path = './testdata/NT_ENZYME.pkl'
split_data(data_path, type, filter_num)

data_path = f'./ENZYME_{type}.pkl'
