...
def get_dataloader(dist_map, id_ec, ec_id, args):
    ...
    negative = mine_hard_negative(dist_map, 30) # dataloader 中
...
def main():
    ...
    # id_ec, ec_id_dict = get_ec_id_dict('./data/' + args.training_data + '.csv') # len=8151,len=3082
    
    csv_path = './data/' + args.training_data + '.csv'
    prot_ids = batch['prot_ids']  # 假设这是您的 prot_ids 列表
    selected_rows = extract_rows(csv_path, prot_ids)
    id_ec, ec_id_dict = get_ec_id_dict_by_rows(selected_rows)
    ...
    for epoch in range(1, epochs + 1):
        if epoch % args.adaptive_rate == 0 and epoch != epochs + 1:
            ...
            if epoch != args.adaptive_rate:
                os.remove('./data/model/' + model_name + '_' + str(epoch-args.adaptive_rate) + '.pth')
