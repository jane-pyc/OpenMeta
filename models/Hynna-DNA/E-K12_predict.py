...
def batch_data(emb_f, contig_to_prots_f, batch_data_path, batch_data_prot_index_dict_path):
    ...
    for key,val in esm_embs_DNA.items():
        all_prot_ids.append(key)  #len=4304

    emb_f = emb_f
    with open(emb_f, 'rb') as emb_file:
        esm_embs = pk.load(emb_file)
    embs = []
    for key, val in esm_embs.items():
        embs.append(val.cpu().numpy() if isinstance(val, torch.Tensor) else val)
    embs = np.array(embs, dtype=np.float16)  # [4730,128]
...
def predict(y_test=False):
    ...
    emb_f = './testdata/Hyena_tiny_Operons.pkl'
    contig_to_prots_f = new_meta_file_path
    batch_data_path = './testdata/Hyena_Operons_batchdata.pkl'
    batch_data_prot_index_dict_path = './testdata/Hyena_Operons_protindex.pkl'
