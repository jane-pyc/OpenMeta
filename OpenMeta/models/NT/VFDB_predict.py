...
def split_data(data_path, type, filter_num):      
    path = './testdata/VFDB_esm2-1.pkl'
    data = pk.load(open(path,"rb"))
    keys=[]
    for key, _ in data.items():
            keys.append(key)
    keys_stack = np.stack(keys) #(1965,)
    ...
    X = values_stack #(1,1175, 4107)
    X = X[0]
    y = keys_stack #(4236)
    y = y[:1175]
    
    csv_path = '../../../src/datasets/Large-Scale/VFDB/VFDB_num.csv'  
    df = pd.read_csv(csv_path)
    df = df.drop_duplicates(subset='VF GID', keep='first')
    df['VF GID'] = df['VF GID'].astype(str)
    y_output = df.set_index('VF GID').loc[y]['VF Cate Code'].values  
    ...
    train_split_path = "./VFDB_"+type+".pkl"
...
def train_ML(data_path):  
    ...
    y_output = df.set_index('VF GID').loc[y]['VF Cate Code'].values #(4230,)       
    y = y_output    
...
def train(data_path, type, filter_num, report_logger):
    ...
    precision["micro"], recall["micro"], _ = precision_recall_curve(
        y_bin.ravel(), np.array(y_pred_proba).ravel() #ravel返回一个1D数组。
    )
    ...
    report_logger.info("Dataset: VFDB")
    ...
    result_path = os.path.join(report_dir, f'VFDB_{type}_classification_results.csv')
    
    results_df.to_csv(result_path, index=False)
    original_df = pd.read_csv('../../../src/datasets/Large-Scale/VFDB/VFDB_code.csv') 
    encoded_df = pd.read_csv('../../../src/datasets/Large-Scale/VFDB/VFDB_num.csv') 
    code_to_name = dict(zip(encoded_df['VF Cate Code'], original_df['VF CID']))
    results_df = pd.read_csv(result_path)
    results_df['Class Name'] = results_df['Class Name'].astype(int).map(code_to_name)
    
    result_path_updated = f'{figure_path}/classification_report/VFDB_{type}_classification_results_updated.csv'
...
data_path = './testdata/NT_VFDB.pkl'
split_data(data_path, type, filter_num)
# train_ML(data_path)
data_path = f'./VFDB_{type}.pkl'
