...
def prepare_data(path, config):
	...
	data = pd.read_csv(path + '/abundance.tsv', index_col=0, sep='\t', header=None) # [606, 440]
	labels = np.genfromtxt(path + '/labels.txt', dtype=np.str_, delimiter=',')		# [440]
	...
	data = data.divide(sums, axis=0) # normalization

	labels, label_set = pd.factorize(labels)   # labels: list; label_set: set


	# only select partial feature in the raw features. select the features whose value are large than threshold.
	# 606 --> 216
	...
	# data: dictionary
	# key: value
	# the loop of for to construct a tree for each sample. 
	# features_df: the dataframe: key: value --> special:range
