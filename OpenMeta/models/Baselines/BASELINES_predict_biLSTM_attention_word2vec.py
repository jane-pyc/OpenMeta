...
class BiLSTMAttention(nn.Module):
    ...
    def forward(self, inputs):
        ...
        hidden_states, _ = self.bilstm(embeddings)
...
def predict(y_test=False):
    ...
    for seq in sequences:
        encoded_seq = [vocab[aa] for aa in seq if aa in vocab]
        padded_seq = encoded_seq[:max_sequence_length]
        pad_length = max(0, max_sequence_length - len(padded_seq))
        padded_seq += [0] * pad_length
    ...
    model_path = 'model_biLSTM/mgy_proteins_222_pretrained_bilstm_attention_word2vec_inputsize_100_model_new.pth'
    pretrained_model_path = model_path
    input_size = 100 
    hidden_size = 256
    num_layers = 2 
    dropout = 0.2 
    pretrained_model = BiLSTMAttention(input_size, hidden_size, num_layers, dropout)
    ...
    for pkl_f in test_pkls:
        ...
        for batch in loop:
            ...
            inputs_embeds = inputs_embeds.view(100, 30*32)
